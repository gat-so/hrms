...
# Usage: deploy-preview.sh <pr_number> <repo_url> <branch> <preview_domain>
set -e
...
if [ -z "$PR_NUM" ] || [ -z "$REPO_URL" ] || [ -z "$BRANCH" ]; then
    echo "Usage: deploy-preview.sh <pr_number> <repo_url> <branch> [preview_domain]"
    exit 1
fi
